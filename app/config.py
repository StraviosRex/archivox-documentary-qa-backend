...
def list_public_profiles() -> list[dict[str, str]]:
    """Return frontend-safe profile metadata. No API keys."""
...
def resolve_llm_profile(profile_name: str | None = None) -> dict[str, Any]:
    ...
    If profile_name is None, use LLM_PROFILE from .env.
    Manual overrides still apply only to the default .env-selected profile.
    Per-request frontend profile selection uses the selected profile as-is.
    """
    profiles = load_model_profiles()
